...
class HarboursRemoteDataSource {
    ...
    suspend fun fetchTextFromUrlByBoundingBox(bounds: LatLngBounds, zoomLevel: Double): String {
        val sb = StringBuilder(baseUrl)
        sb.append("b=${bounds.latitudeSouth}")
            .append("&t=${bounds.latitudeNorth}")
            .append("&l=${bounds.longitudeWest}")
            .append("&r=${bounds.longitudeEast}")
            .append("&ucid=0")
            .append("&maxSize=5")
