...
class PoiUtil {
    ...
    fun extractCategoryFromPoiEntity(tagsMap: Map<String, String>): String? {
        return when {
            ...
            tagsMap.containsKey("bus") && tagsMap.containsKey("railway") -> if (tagsMap["railway"] != "stop") "bus_stop & ${tagsMap["railway"]}" else "bus"
    ...
    fun unifyPoiCategory(categoryName: String): List<String> {
        val unifyCategoryMap = mapOf(
            ...
            "Health" to listOf("Hospital", "Dentist", "Doctors", "Pharmacy", "First aid", "Clinic"),
        ...
        for (map in unifyCategoryMap) {
            // if category name is equal to map key return list of string
            if (map.key == categoryName) return map.value
            // if list of string contains category name return it's map key
            else if (map.value.contains(categoryName)) return listOf(map.key)
        }
    ...
    fun generalizeTagKeys(elementList: List<OverpassNodeModel>?): List<OverpassNodeModel> {
        val stringMap = mapOf(
            "payment" to mapOf(
                "Credit cards" to listOf(
                    "card",
                    "cards",
                    "contactless",
                    "credit_cards",
